...
<table width="100%" border="0" cellpadding="2" cellspacing="1" bgcolor="#22B5DD">
  ...
  <tr bgcolor="#FFFFFF"> 
    ...
    $strHasil=mysqli_query($koneksi, "SELECT * FROM analisa_hasil,penyakit_solusi WHERE analisa_hasil.id_pasien='$id_pasien' AND analisa_hasil.kd_penyakit=penyakit_solusi.kd_penyakit ORDER BY analisa_hasil.persentase DESC ");
    //$strHasil=mysql_query("SELECT * FROM analisa_hasil,penyakit_solusi WHERE analisa_hasil.id_pasien='$id_pasien' AND analisa_hasil.kd_penyakit=penyakit_solusi.kd_penyakit ORDER BY analisa_hasil.persentase DESC ");
  while($dataHasil=mysqli_fetch_array($strHasil)){
	...
	?></td>
    <td><?php echo $data['tanggal']; ?>&nbsp;|<a title="hapus pengguna" style="cursor:pointer;" onClick="return konfirmasi('<?php echo $data['id_pasien'];?>')"><img src="image/hapus.jpg" width="16" height="16" ></a></td>
  </tr>
  <?php
  }
  ?>
